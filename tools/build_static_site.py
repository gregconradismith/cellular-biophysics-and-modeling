...
OUT = ROOT / "docs"
SITE_URL = "https://cellularbiophysicsandmodeling.com"
NS = {
...
def rewrite_links(content: str, from_dir: str, by_slug: dict[str, Entry]) -> str:
    ...
    return re.sub(r"""((?:href|src)=["'])(https?://cellularbiophysicsandmodeling\.com/[^"']+)(["'])""", repl, content)
...
def page_shell(title: str, body: str, from_dir: str, metadata: dict[str, str]) -> str:
    ...
    calendar = relative_url(from_dir, "pages/fall-2025-calendar/").rstrip("/")
...
def render_home(metadata: dict[str, str], entries: list[Entry]) -> None:
    ...
    lectures = sorted((e for e in pages if e.slug.startswith("lecture-")), key=lecture_key)
    featured_slugs = ["about", "fall-2025-calendar", "syllabus", "readings", "slides", "videos", "the-polyqual-game"]
    featured = [e for slug in featured_slugs for e in pages if e.slug == slug]
    body = f"""    <section class="hero">
      <div class="hero-copy">
        <p class="eyebrow">William & Mary neuroscience course materials</p>
        ...
        <div class="hero-actions">
          <a class="button primary" href="pages/fall-2025-calendar/">View calendar</a>
          <a class="button" href="pages/">Browse materials</a>
        </div>
      </div>
      <img src="https://cellularbiophysicsandmodeling.com/wp-content/uploads/2018/11/cbpprimergraphicrectangle-001.jpeg" alt="Cellular Biophysics and Modeling primer artwork">
    </section>
    ...
    <section class="section two-column">
      <div>
        <div class="section-heading">
          <p class="eyebrow">{len(lectures)} lecture pages</p>
          <h2>Lecture sequence</h2>
        </div>
        <ol class="link-list">
          {''.join(f'<li><a href="{relative_url("", output_path(e)).rstrip("/")}">{html.escape(e.title)}</a><span>{html.escape(excerpt(e, 12))}</span></li>' for e in lectures)}
        </ol>
      </div>
...
def render_index(metadata: dict[str, str], entries: Iterable[Entry], kind: str) -> None:
    ...
    if kind == "post":
        ...
        ordered = sorted(entries, key=lambda e: (0 if e.slug.startswith("lecture-") else 1, lecture_key(e), e.title))
        title = "Course Materials"
        subtitle = "Pages, lectures, readings, slides, videos, and helper resources."
